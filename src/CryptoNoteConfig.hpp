...
// this commit is modified to be tested on CRR (CryptoRial) network.
#pragma once
...
const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX = 0x2756;  // Cryptorial addresses start with "cr"
...
const unsigned EMISSION_SPEED_FACTOR = 21; //CryptoRial is planned to grow slower
static_assert(EMISSION_SPEED_FACTOR <= 8 * sizeof(uint64_t), "Bad EMISSION_SPEED_FACTOR");

const size_t CRYPTONOTE_REWARD_BLOCKS_WINDOW = 100;
const size_t CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE =
    44576;  // size of block (bytes) after which reward for block calculated using block size
